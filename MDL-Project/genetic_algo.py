...
def storebest_10(p_v,p_e,p_per):
    ...
    B10_V = vectors[0:10]
    B10_E = errors[0:10]
    name = "./generations"+str(cur_generation)
...
def mutation(p_vector):
    ...
    for i in range(0,num.shape[0]):
        if num[i]<35:
            ...
            if abs(temp-p_vector[i][place]) < 0.0001 :
                temp += uniform(-2,2)
...
if __name__ == "__main__":
    ...
    with open('output.json','r') as file:
        data = json.load(file)
    Generations[cur_generation]['vectors'] = np.array(data)
    Generations[cur_generation]['vectors_error'] = np.array(get_population_errors(data))

    normal_error = Generations[cur_generation]['vectors_error'][:,0] * (2/3) + Generations[cur_generation]['vectors_error'][:,1] *(1/3)
    Generations[cur_generation]['vectors_percent'] = np.reciprocal(normal_error+1)
